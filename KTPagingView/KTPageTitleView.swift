...
class KTPageTitleButton: UIButton {
}

protocol KTPageTitleViewDelegate : NSObjectProtocol {
    /// index 当前选中标题对应的下标值
    func pageTitleView(pageTitleView: KTPageTitleView, index: Int)
}

class KTPageTitleView: UIView {
    // MARK: - 给外界属性
    /// 选中标题按钮下标，默认为 0
    var index: Int = 0
    ...
    var resetIndex: Int?
    /// 拓展 init 方法
    init(frame: CGRect, delegate: KTPageTitleViewDelegate, titleNames: [String], configure: KTPageTitleViewConfigure) {
    ...
    // MARK: - 私有属性
    private weak var delegateTitleView : KTPageTitleViewDelegate?
    private var titleNames: [String] = []
    private var configure: KTPageTitleViewConfigure!
    private var allTitleTextWidth: CGFloat = 0.0
    private var allWidth: CGFloat = 0.0
    private var btnMArr:[KTPageTitleButton] = []
...
extension KTPageTitleView {
    ...
    func setPageTitleView(progress: CGFloat, originalIndex: Int, targetIndex: Int) {
        ...
        // 3、处理指示器的逻辑
        if allWidth <= self.bounds.size.width { /// KTPageTitleView 静止样式
